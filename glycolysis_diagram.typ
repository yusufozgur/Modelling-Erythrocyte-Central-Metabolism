...
#let diagram_glycolysis = diagram(
  ...
  node(
    (rel: (.4,1.2)), "GA3P", corner-radius: 2pt
  ),
  edge("<|-|>"),
  node(
    (rel: (-.7,0)), "DHAP", corner-radius: 2pt
  ),
  node(
    (rel: (.3,-.7)), "Aldolase", stroke: 0pt
  ),
  node(
    (rel: (0,1.2)), 
  ...
  edge("-|>", label:[#set align(center);Pyruvate Kinase#linebreak() (ADP -> ATP)],
  label-side: left),
  node(
    (rel: (0,1.5)),
    "Pyruvate"
  ),
  edge("<|-|>", label:[#set align(center);Lactate Dehydrogenase#linebreak() (NADH -> NAD+)],
  label-side: left),
  node(
    (rel: (0,1.5)),
    "Lactate"
